...
class ExampleBot(HackathonBot):
    ...
    # function to generate visiblities from every tile
    def calculate(self):
    ...
    # returns visible tiles, currently not used
...
#---- actions
    ...
    def zone_fighter(self, game_state: GameState):
        if len(self.enemies):
            ...
            next_pos = self.shoot_tile(game_state, list(self.enemies.values())[0].position)
        else:
...
# --- actions end
    ...
    def decide_action(self, game_state: GameState):
        if self.fight_started:
            ...
            if zone and zone.status==ZoneStatus.CAPTURED:
